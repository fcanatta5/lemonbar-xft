...
B<lemonbar> (formerly known as B<bar>) is a lightweight bar entirely based on XCB. This Xft-enabled fork provides UTF-8 support, Fontconfig/Xft fonts, basic formatting, RandR and optional Xinerama support, and EWMH compliance without wasting your precious memory.

=head1 DEPENDENCIES

Building this Xft-enabled fork requires a C11 compiler, C<pkg-config>, and development headers for XCB, RandR, Xlib/XCB, Xft, Fontconfig and FreeType. On Debian/Ubuntu-like systems:

    sudo apt-get install pkg-config libxcb1-dev libxcb-randr0-dev libx11-dev libx11-xcb-dev libxft-dev libfontconfig1-dev libfreetype6-dev

Xinerama support is optional. Build with C<make WITH_XINERAMA=1> and install C<libxcb-xinerama0-dev> if you need it. Use C<make check> for a minimal command-line sanity check that does not require an X display.
