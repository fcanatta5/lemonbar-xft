...
static bool sel_fg_allocated = false;
static XftDraw *xft_draw;

// Per-font Xft glyph width cache. Widths differ between font slots, so a
// character-only cache gives wrong positioning when fallback fonts are used.
#define MAX_WIDTHS (1 << 16)
static bool xft_width_cached[MAX_FONT_COUNT][MAX_WIDTHS];
static int  xft_width[MAX_FONT_COUNT][MAX_WIDTHS];

static const rgba_t COLOR_TRANSPARENT_BLACK = { .r = 0x00, .g = 0x00, .b = 0x00, .a = 0x00 };
static const rgba_t COLOR_OPAQUE_WHITE = { .r = 0xff, .g = 0xff, .b = 0xff, .a = 0xff };

void update_gc (void);

static uint8_t
scale_alpha (uint8_t component, uint8_t alpha)
{
    return (uint8_t)(((unsigned int)component * alpha) / 255u);
}
...
make_rect (int x, int y, int width, int height)
{
    return (xcb_rectangle_t){ to_i16(x), to_i16(y), to_u16(width), to_u16(height) };
}

static void
reset_formatting_state (void)
{
    fgc = dfgc;
    bgc = dbgc;
    ugc = dugc;
...
static bool
set_xft_color (rgba_t color_value)
{
    char color[] = "#ffffff";
    uint32_t rgb = color_value.v & 0x00ffffff;

    snprintf(color, sizeof(color), "#%06X", rgb);

    if (sel_fg_allocated) {
        XftColorFree(dpy, visual_ptr, colormap, &sel_fg);
        sel_fg_allocated = false;
    }
...
xcb_void_cookie_t xcb_poly_text_16_simple(xcb_connection_t * c,
    ...
    uint8_t len, const uint16_t *str)
{
...
static int
xft_glyph_width_uncached (uint32_t ch, font_t *cur_font)
{
    XGlyphInfo gi;
    FT_UInt glyph = XftCharIndex(dpy, cur_font->xft_ft, (FcChar32)ch);

    if (!glyph)
        return cur_font->width;
    ...
    return (gi.xOff >= gi.width) ? gi.xOff : gi.width;
}

int
xft_char_width (uint32_t ch, font_t *cur_font)
{
    const int font_slot = cur_font->cache_index;

    if (font_slot < 0 || font_slot >= MAX_FONT_COUNT)
        return cur_font->width;

    if (ch >= MAX_WIDTHS)
        return xft_glyph_width_uncached(ch, cur_font);

    if (!xft_width_cached[font_slot][ch]) {
        xft_width[font_slot][ch] = xft_glyph_width_uncached(ch, cur_font);
        xft_width_cached[font_slot][ch] = true;
    }

    return xft_width[font_slot][ch];
}
    ...
    switch (align) {
        case ALIGN_C:
            xcb_copy_area(c, mon->pixmap, mon->pixmap, gc[GC_DRAW],
                    to_i16(mon->width / 2 - x / 2), 0,
                    to_i16(mon->width / 2 - (x + ch_width) / 2), 0,
                    to_u16(x), to_u16(bh));
            x = mon->width / 2 - (x + ch_width) / 2 + x;
            break;
        case ALIGN_R:
            xcb_copy_area(c, mon->pixmap, mon->pixmap, gc[GC_DRAW],
                    to_i16(mon->width - x), 0,
                    to_i16(mon->width - x - ch_width), 0,
                    to_u16(x), to_u16(bh));
...
draw_char (monitor_t *mon, font_t *cur_font, int x, int align, uint32_t ch)
{
    ...
    if (cur_font->xft_ft) {
        if (xft_draw && sel_fg_allocated) {
            FcChar32 glyph_char = (FcChar32)ch;
            XftDrawString32(xft_draw, &sel_fg, cur_font->xft_ft, x, y, &glyph_char, 1);
        }
    } else {
        uint16_t ucs2 = (uint16_t)ch;

        /* xcb accepts string in UCS-2 BE, so swap */
        ucs2 = (uint16_t)((ucs2 >> 8) | (ucs2 << 8));
...
parse_color (const char *str, char **end, const rgba_t def)
{
    ...
    string_len = (size_t)(ep - (str + 1));

    switch (string_len) {
        case 3:
            r = (uint8_t)(((parsed >> 8) & 0x0fu) * 0x11u);
            g = (uint8_t)(((parsed >> 4) & 0x0fu) * 0x11u);
            b = (uint8_t)(( parsed       & 0x0fu) * 0x11u);
            a = 0xff;
            break;
        case 6:
            r = (uint8_t)((parsed >> 16) & 0xffu);
            g = (uint8_t)((parsed >> 8)  & 0xffu);
            b = (uint8_t)( parsed        & 0xffu);
            a = 0xff;
            break;
        case 8:
            a = (uint8_t)((parsed >> 24) & 0xffu);
            r = (uint8_t)((parsed >> 16) & 0xffu);
            g = (uint8_t)((parsed >> 8)  & 0xffu);
            b = (uint8_t)( parsed        & 0xffu);
            break;
...
{
    const char *attr = strchr("ou", attribute);
...
area_add (char *str, const char *optend, char **end, monitor_t *mon, const int x, const int align, const int button)
{
    ...
    if (*str != ':') {
        *end = str;
        ...
        if (i < 0) {
            fprintf(stderr, "Tried to close a clickable area that was never opened\n");
            return false;
        }
    ...
    *trail = '\0';
    ...
    for (char *needle = str; *needle; needle++) {
        size_t delta = (size_t)(trail - &needle[1]);
...
font_has_glyph (font_t *font, const uint32_t c)
{
    ...
    if (c > UINT16_MAX || c < font->char_min || c > font->char_max)
        return false;
...
static bool
is_utf8_continuation (uint8_t byte)
{
    return (byte & 0xc0u) == 0x80u;
}

static uint32_t
utf8_next_codepoint (char **cursor)
{
    uint8_t *utf = (uint8_t *)*cursor;
    uint32_t cp;
    ...
    if ((utf[0] & 0xe0u) == 0xc0u) {
        if (!is_utf8_continuation(utf[1]))
            goto invalid;

        cp = ((uint32_t)(utf[0] & 0x1fu) << 6) |
             ((uint32_t)(utf[1] & 0x3fu));
        if (cp < 0x80u)
            goto invalid;
    ...
    if ((utf[0] & 0xf0u) == 0xe0u) {
        if (!is_utf8_continuation(utf[1]) || !is_utf8_continuation(utf[2]))
            goto invalid;

        cp = ((uint32_t)(utf[0] & 0x0fu) << 12) |
             ((uint32_t)(utf[1] & 0x3fu) << 6) |
             ((uint32_t)(utf[2] & 0x3fu));
    ...
    if ((utf[0] & 0xf8u) == 0xf0u) {
        if (!is_utf8_continuation(utf[1]) ||
            !is_utf8_continuation(utf[2]) ||
            !is_utf8_continuation(utf[3]))
            goto invalid;

        cp = ((uint32_t)(utf[0] & 0x07u) << 18) |
             ((uint32_t)(utf[1] & 0x3fu) << 12) |
             ((uint32_t)(utf[2] & 0x3fu) << 6) |
             ((uint32_t)(utf[3] & 0x3fu));
...
parse (char *text)
{
    ...
    // Reset per-line parser state. Formatting from one input line should not
    // leak into the next one.
    area_stack.at = 0;
    reset_formatting_state();
    ...
    for (;;) {
        ...
        if (p[0] == '%' && p[1] == '{' && (block_end = strchr(p++, '}'))) {
            ...
            uint32_t ucs = utf8_next_codepoint(&p);
...
font_load (const char *pattern)
{
    ...
    ret->cache_index = font_count;

    size_t pattern_len = strlen(pattern);
    if (pattern_len > UINT16_MAX) {
        fprintf(stderr, "Font name is too long: %s\n", pattern);
        free(ret);
        return;
    }

    cookie = xcb_open_font_checked(c, font, (uint16_t)pattern_len, pattern);
    xcb_generic_error_t *font_error = xcb_request_check(c, cookie);
    if (!font_error) {
        queryreq = xcb_query_font(c, font);
        font_info = xcb_query_font_reply(c, queryreq, NULL);

        if (!font_info) {
            fprintf(stderr, "Could not query font %s\n", pattern);
        ...
        size_t lut_size = sizeof(xcb_charinfo_t) * (size_t)xcb_query_font_char_infos_length(font_info);
        if (lut_size) {
            ret->width_lut = malloc(lut_size);
            if (!ret->width_lut) {
                fprintf(stderr, "Could not allocate glyph width table for font %s\n", pattern);
                free(font_info);
                xcb_close_font(c, font);
                free(ret);
                return;
...
set_ewmh_atoms (void)
{
    ...
    for (int i = 0; i < atoms; i++)
        atom_cookie[i] = xcb_intern_atom(c, 0, (uint16_t)strlen(atom_names[i]), atom_names[i]);
...
monitor_new (int x, int y, int width, int height)
{
    ...
    xcb_create_window(c, to_u8(depth), ret->window, scr->root,
                      to_i16(ret->x), to_i16(ret->y), to_u16(width), to_u16(bh), 0,
    ...
    xcb_create_pixmap(c, to_u8(depth), ret->pixmap, ret->window, to_u16(width), to_u16(bh));
...
{
    const xcb_rectangle_t *r1 = (const xcb_rectangle_t *)p1;
    const xcb_rectangle_t *r2 = (const xcb_rectangle_t *)p2;
...
get_visual (void)
{
    ...
    if (result > 0 && result_ptr) {
        xcb_visualid_t visual_id = (xcb_visualid_t)result_ptr->visualid;
...
// Parse a basic X-style geometry string: [=][width][xheight][+xoffset][+yoffset].
// Omitted fields keep their current defaults in tmp. Negative offsets are not
// supported because lemonbar interprets y differently for top and bottom bars.
bool
parse_geometry_string (char *str, int *tmp)
{
    ...
    while (*p) {
        ...
        errno = 0;
        unsigned long parsed = strtoul(p, &p, 10);
        if (errno || parsed > INT_MAX) {
            fprintf(stderr, "Invalid geometry specified\n");
            return false;
        }

        tmp[field] = (int)parsed;
    }
...
init (char *wm_name, char *wm_instance)
{
    ...
    for (monitor_t *mon = monhead; mon; mon = mon->next) {
        ...
        if (wm_name)
            xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, (uint32_t)strlen(wm_name), wm_name);

        // set the WM_CLASS atom instance to the executable name
        if (wm_instance) {
            char *wm_class;
            size_t wm_class_offset, wm_class_len;
            ...
            if (!wm_class) {
                fprintf(stderr, "Could not allocate WM_CLASS string\n");
                continue;
            }
            ...
            xcb_change_property(c, XCB_PROP_MODE_REPLACE, mon->window, XCB_ATOM_WM_CLASS, XCB_ATOM_STRING, 8, (uint32_t)wm_class_len, wm_class);
...
static bool
parse_int_arg (const char *value, int min_value, int max_value, int *out)
{
    char *end = NULL;
    long parsed;
    ...
    if (errno || end == value || *end != '\0' || parsed < min_value || parsed > max_value)
        return false;
...
static void
usage (const char *program, FILE *stream)
{
    fprintf(stream,
            "lemonbar version %s patched with XFT support\n"
            "usage: %s [-h] [-g geometry] [-b] [-d] [-f font] [-a areas] "
            "[-p] [-n name] [-u pixels] [-B color] [-F color] [-U color] [-o offset]\n"
            "\t-h Show this help\n"
            "\t-g Set the bar geometry {width}x{height}+{xoffset}+{yoffset}\n"
            "\t-b Put the bar at the bottom of the screen\n"
            "\t-d Force docking (use this if your WM isn't EWMH compliant)\n"
            "\t-f Set a font name to use; repeat up to %d times\n"
            ...
            "\t-B Set background color (#rgb, #rrggbb, #aarrggbb)\n"
            "\t-F Set foreground color (#rgb, #rrggbb, #aarrggbb)\n"
            "\t-U Set underline/overline color (#rgb, #rrggbb, #aarrggbb)\n"
            "\t-o Add a vertical offset to the text; it can be negative\n",
            VERSION, program, MAX_FONT_COUNT);
}
...
main (int argc, char **argv)
{
    ...
    char *font_patterns[MAX_FONT_COUNT] = {0};
    int font_pattern_count = 0;

    // B/W combo
    dbgc = bgc = COLOR_TRANSPARENT_BLACK;
    dfgc = fgc = COLOR_OPAQUE_WHITE;
    ...
    while ((ch = getopt(argc, argv, "hg:bdf:a:pu:B:F:U:n:o:")) != -1) {
        switch (ch) {
            case 'h':
                ...
                return EXIT_SUCCESS;
            case 'g':
                if (!parse_geometry_string(optarg, geom_v))
                    return EXIT_FAILURE;
                break;
            ...
            case 'f':
                if (font_pattern_count >= MAX_FONT_COUNT) {
                    fprintf(stderr, "Max font count reached. Could not load font \"%s\"\n", optarg);
                    return EXIT_FAILURE;
                }
                font_patterns[font_pattern_count] = strdup(optarg);
                if (!font_patterns[font_pattern_count]) {
                    perror("strdup");
                    return EXIT_FAILURE;
                }
                font_pattern_count++;
                break;
            case 'u':
                if (!parse_int_arg(optarg, 0, INT_MAX, &parsed)) {
                    fprintf(stderr, "Invalid underline/overline height: %s\n", optarg);
                    return EXIT_FAILURE;
                }
                bu = parsed;
                break;
            case 'o':
                if (!parse_int_arg(optarg, INT_MIN, INT_MAX, &parsed)) {
                    fprintf(stderr, "Invalid vertical offset: %s\n", optarg);
                    return EXIT_FAILURE;
                }
                add_y_offset(parsed);
                break;
            case 'B': dbgc = bgc = parse_color(optarg, NULL, COLOR_TRANSPARENT_BLACK); break;
            case 'F': dfgc = fgc = parse_color(optarg, NULL, COLOR_OPAQUE_WHITE); break;
            case 'U': dugc = ugc = parse_color(optarg, NULL, fgc); break;
            case 'a':
                if (!parse_int_arg(optarg, 0, INT_MAX, &parsed)) {
                    fprintf(stderr, "Invalid clickable area count: %s\n", optarg);
                    return EXIT_FAILURE;
                }
                areas = parsed;
                break;
    ...
    if (areas) {
        area_stack.area = calloc((size_t)areas, sizeof(area_t));
    ...
    // Connect to the X server only after option parsing. This keeps -h and
    // invalid CLI invocations useful on machines without DISPLAY.
    xconn();

    for (int i = 0; i < font_pattern_count; i++) {
        font_load(font_patterns[i]);
        free(font_patterns[i]);
        font_patterns[i] = NULL;
    }
    ...
    for (;;) {
        ...
        int poll_status = poll(pollin, 2, -1);
        if (poll_status < 0) {
            if (errno == EINTR)
                continue;
            perror("poll");
        ...
        if (redraw) { // Copy our temporary pixmap onto the window
            for (monitor_t *mon = monhead; mon; mon = mon->next) {
                xcb_copy_area(c, mon->pixmap, mon->window, gc[GC_DRAW], 0, 0, 0, 0, to_u16(mon->width), to_u16(bh));
            }
        }
